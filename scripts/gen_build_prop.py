...
def get_build_keys(product_config):
  ...
  if default_cert == "" or default_cert == os.path.join(TEST_KEY_DIR, "testKey"):
    return "test-keys"
  if default_cert.startswith("vendor/euclid-priv/"):
    return "release-keys"
  return "dev-keys"
...
def parse_args():
  ...
  config["EuclidDesc"] = config["BuildDesc"]
  config["EuclidDevice"] = config["DeviceName"]
...
def generate_build_info(args):
  ...
  if config["BuildVariant"] == "user":
    ...
    print(f"ro.build.display.id?={config['EuclidDesc']}")
  ...
  print(f"ro.euclid.device={config['EuclidDevice']}")
